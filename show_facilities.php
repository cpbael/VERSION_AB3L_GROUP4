...
<?php
	$services_array=mysql_query("select * from service where type_id IN (SELECT type_id from type where classification='FACILITY')group by type_id");
	$services=array();
	...
	$col=1;
		...
		while($i<count($services)){
			...
			$j=0;
				...
				echo "{$type['article']}";
				echo"<h3>No. of Facilities: {$count[0]}</h3>";
