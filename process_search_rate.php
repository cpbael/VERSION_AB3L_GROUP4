...
<?php
    $rate = $_GET['rate']; 
        ...
        $raw_results = mysql_query("SELECT * FROM service
			...
			echo "<div class = log4>		
						<form method=get action=process_search_rate.php id=search>			
							<center>
							<table>
								<tr><td><h1 class = h1>SEARCH BY RATE &nbsp</h1></td>
								<td><input name=rate type=text size=50 value={$rate} pattern = '[0-9]*[0-9]*[0-9]' placeholder='Search By Rate' /></center></td></tr>
							</table>
						</form>
					<br/><br/>
				<div>";
        if(mysql_num_rows($raw_results) > 0){ 
            while($results = mysql_fetch_array($raw_results)){
				...
				echo "<hr/>{$type['article']}<hr/>";
				echo"<br/><h3>No. of Rooms: {$count[0]}</h3>";
				...
				echo "<br/><input id = mysubmit3 type=button onclick=window.location.href='add_reservation.php?type_id={$results['type_id']}' value=Reserve>";
