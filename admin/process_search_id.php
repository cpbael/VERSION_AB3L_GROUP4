...
<html>
	...
	<tr>
		<td colspan = "9"><h1><center>RESULTS</center></h1></td>
	</tr>
	<tr><center>
		<th>NAME</th>
		<th>CONTACT NO</th>
...
<?php
			...
			$reservation= mysql_fetch_array(mysql_query($query));			///get service_name
			$query2="SELECT service_name from service where service_id={$reservation['service_id']};";
			$service_name= mysql_fetch_array(mysql_query($query));	
			if($reservation['is_Member']==1){
				$res=mysql_fetch_array(mysql_query("SELECT fullname, contactno from member where member_id='{$reservation['member_id']}';"));
				$reserver=$res[0];
			}else{
				$res=mysql_fetch_array(mysql_query("SELECT fullname, contactno from guest where guest_id='{$reservation['member_id']}';"));
				$reserver=$res[0];
			}
			...
			echo "<td class = allTextBlack>",$reservation['reservation_id'], "</td>";
			echo "<td class = allTextBlack>",$service_name[0], "</td>";
			echo "<td class = allTextBlack>{$reservation['start_date']}</td>";
			echo "<td class = allTextBlack>{$reservation['end_date']}</td>";
			echo "<td class = allTextBlack>{$reservation['price']}</td>";
			//echo "<td class = allTextBlack><a href='edit_reservation.php?reservation_id={$reservation[$i]['reservation_id']}'>Edit</a></td>";
			
			
			
			if($reservation['status']==-1){
				echo "<td><input id = mysubmit5 type=button onclick=window.location.href='edit_reservation.php?reservation_id={$reservation['reservation_id']}' value=Edit></center></td>";
			}
			if($reservation['status']==-1){
				echo "<td><input  id = mysubmit5 type=button onclick=window.location.href='process_check_in.php?reservation_id={$reservation['reservation_id']}' value= 'Check In'></td>";
			}
			if($reservation['status']==0){
				echo "<td><input  id = mysubmit5 type=button onclick=window.location.href='process_check_out.php?reservation_id={$reservation['reservation_id']}' value= 'Check Out'></td>";
			}
			echo "</tr>";
